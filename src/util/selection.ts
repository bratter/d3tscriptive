 ...
 */
export type SimpleSelection<GElement extends BaseType = BaseType>
  = Selection<GElement, any, any, any>;

/**
 * Simple TransitionLike type.
 * Provides basic types to the TransitionLike generic, including a default element of BaseType.
 */
export type SimpleTransitionLike<GElement extends BaseType = BaseType>
  = TransitionLike<GElement, any>;

/**
 * Simple Transition type.
 * Provides basic types to the Transition generic, including a default element of BaseType.
 */
export type SimpleTransition<GElement extends BaseType = BaseType>
  = Transition<GElement, any, any, any>;
 ...
 */
// export type SimpleContext<GElement extends BaseType> =
//   SimpleSelection<GElement> | SimpleTransitionLike<GElement>
export type SimpleContext<GElement extends BaseType = BaseType>
  = SimpleSelection<GElement> | SimpleTransition<GElement>;

/**
 * Returns a Selection when passed a Context.
 * Used to get the selection inside transition-safe reusable chart objects.
 */
export function selectionFromContext<GElement extends BaseType = BaseType>(
  context: SimpleContext<GElement>,
): SimpleSelection<GElement> {
  // return (<SimpleTransitionLike<GElement>>context).selection
  //   ? (<SimpleTransitionLike<GElement>>context).selection()
  return (context as SimpleTransition<GElement>).selection
    ? (context as SimpleTransition<GElement>).selection()
    : context as SimpleSelection<GElement>;
}
